            ...
            if dockable == True and not self.isDockable():
                ...
                if floating == True or area == None:
                    ...
                    if self.parent() is None or self.parent() == omui.MQtUtil.mainWindow():
                        ...
                        workspaceControlName = cmds.workspaceControl(workspaceControlName, label=self.windowTitle(), retain=retain, loadImmediately=True, dockToMainWindow=(area, False), initialWidth=width, initialHeight=height, requiredPlugin=plugins, requiredControl=controls)
                    else:
